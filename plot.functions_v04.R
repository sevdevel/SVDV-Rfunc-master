...
plot.lat.depth <- function(path.name,experiment.name,var,time.slice=9999.5,lon.slice=NA,factor=1,name=NULL,
                           var.lim=NULL,save.plot=F,save.path=NULL,is.isotope=FALSE,conc.lab=NULL,x.axis=T,y.axis=T){
  ...
  #
  #   is.isotope:       TRUE when isotopes are plotted (values >1000 or <1000 are considered NA)
  ################################################################################################################
  ...
  if(is.null(var.lim)){
    var.lim <- range(new.int.matrix*factor,na.rm=T)
  }
  
  new.int.matrix[(new.int.matrix*factor<min(var.lim))] <- min(var.lim)/factor
  new.int.matrix[(new.int.matrix*factor>max(var.lim))] <- max(var.lim)/factor
  ...
  image2D(x = new.lat, y = new.depth*1e-3, z = new.int.matrix*factor, NAcol = "grey", #log = "c",
          ...
          col = col(100),colkey = list(width = 0.5, cex.axis = 1.0, length = 0.5, 
  ...
  if (x.axis==T){
    axis(side = 1, at = seq(-90, 90, by = 30), labels = seq(-90, 90, by = 30),cex=1.6)#, tcl = -0.2)
  mtext(side=1, text=expression(paste("latitude ",degree*E)),line=2.75,adj=0.5,padj=-0.5,cex = 1.0)  
  }
  
  if (y.axis==T){
  mtext(side=2, text=expression("depth (km)"),line=1.5,adj=0.5,padj=-1,cex = 1.0) 
  }
    ...
    savePlot(filename=paste(experiment.name,name,time.slice,".png",sep=" "),type="png")
    }else (savePlot(filename=paste(save.path,experiment.name,name,time.slice,".png",sep=" "),type="png"))
  }
...
plot.lon.lat <- function(path.name,experiment.name,var,time.slice=9999.5,depth.slice=NA,factor=1,name=NULL,
                         var.lim=NULL,save.plot=F,save.path=NULL,is.isotope=FALSE,conc.lab=NULL,x.axis=T,y.axis=T,
                         correction=NULL){
  ...
  image2D(x = new.lon, y = sin(new.lat*0.0174532925), z = new.int.matrix*factor, NAcol = "grey", #log = "c",
          rasterImage = FALSE, shade = 0.05, main = name, clab = c(conc.lab), 
          col = col(100),colkey = list(width = 0.5, cex.axis = 0.8, length = 0.5, 
                                       dist = 0.02, cex.clab = 0.8, side.clab = 2, line.clab = 0.5), 
          las = 1, xlab = "", ylab = "", xaxt = 'n', yaxt = 'n',
          ylim = sin(c(-90,90)*0.0174532925), xlim = c(-180,180), zlim = var.lim, cex=1.5#, contour=list(nlevels = 5)
  )
  
  if (x.axis == T){
  axis(side = 1, at = seq(-180, 180, by = 90), labels = seq(-180, 180, by = 90),cex = 1.2)#, tcl = -0.2)
  mtext(side=1, text=expression(paste("longitude ",degree*E)),line=3.5,adj=0.5,padj=-0.5,cex = 1.2) 
  }
  if (y.axis == T){
  axis(side = 2, at = sin(c(-90, -30, 0, 30, 90)*0.0174532925), labels = c(-90, -30, 0, 30, 90),cex = 1.2)#, tcl = -0.2)
  mtext(side=2, text=expression(paste("latitude ",degree*N)),line=1.5,adj=0.5,padj=-1,cex = 1.2) 
  }
...
plot.Feproxy <- function(path.name,experiment.name,time.slice=9999.5,name=NULL,
                         var.lim=NULL,save.plot=F,save.path=NULL,x.axis=T,y.axis=T){
  ...
  extr1      <- ncvar_get(ncin, "bio_fpart_POM_FeOOH")
  extr2      <- ncvar_get(ncin, "bio_fpart_FeS2")
  extr3      <- ncvar_get(ncin, "bio_fpart_FeCO3")
  extr4      <- ncvar_get(ncin, "bio_fpart_Fe3Si2O4")
  
  int.matrix1 <- extr1[ , , 16 , time.level]
  int.matrix2 <- extr2[ , , 16 , time.level]
  int.matrix3 <- extr3[ , , 16 , time.level]
  int.matrix4 <- extr4[ , , 16 , time.level]
  ...
  new.int.matrix1 <- cbind(int.matrix1[,1],int.matrix1,int.matrix1[,nrow(int.matrix1)])
  new.int.matrix2 <- cbind(int.matrix2[,1],int.matrix2,int.matrix2[,nrow(int.matrix2)])
  new.int.matrix3 <- cbind(int.matrix3[,1],int.matrix3,int.matrix3[,nrow(int.matrix3)])
  new.int.matrix4 <- cbind(int.matrix4[,1],int.matrix4,int.matrix4[,nrow(int.matrix4)])
  new.lat        <- c(-90,lat,90)
  
  new.int.matrix1 <- rbind(new.int.matrix1[1,],new.int.matrix1,new.int.matrix1[nrow(new.int.matrix1),])
  new.int.matrix2 <- rbind(new.int.matrix2[1,],new.int.matrix2,new.int.matrix2[nrow(new.int.matrix2),])
  new.int.matrix3 <- rbind(new.int.matrix3[1,],new.int.matrix3,new.int.matrix3[nrow(new.int.matrix3),])
  new.int.matrix4 <- rbind(new.int.matrix4[1,],new.int.matrix4,new.int.matrix4[nrow(new.int.matrix4),])
  new.lon        <- c(-190,lon,190)
  ...
  image2D(x = new.lon, y = sin(new.lat*0.0174532925), z = new.int.matrix2/(new.int.matrix1+new.int.matrix2+new.int.matrix3+new.int.matrix4), NAcol = "grey", #log = "c",
          rasterImage = FALSE, shade = 0.05, main = name, clab = c(conc.lab),  
          col = col(100),colkey = list(width = 0.5, cex.axis = 0.8, length = 0.5, 
                                       dist = 0.02, cex.clab = 0.8, side.clab = 2, line.clab = 0.5), 
          las = 1, xlab = "", ylab = "", xaxt = 'n', yaxt = 'n',
          ylim = sin(c(-90,90)*0.0174532925), xlim = c(-180,180), zlim = var.lim, contour=list(levels = c(0.7,0.8),labels = c('ferruginous','euxinic'))
  )
  
  if(x.axis==T){
  axis(side = 1, at = seq(-180, 180, by = 90), labels = seq(-180, 180, by = 90), cex=1.2)#, tcl = -0.2)
  mtext(side=1, text=expression(paste("longitude ",degree*E)),line=2.75,adj=0.5,padj=-0.5,cex = 1.2)  
  }
  if(y.axis==T){
  axis(side = 2, at = sin(seq(-90, 90, by = 30)*0.0174532925), labels = seq(-90, 90, by = 30), cex=1.2)#, tcl = -0.2)
  mtext(side=2, text=expression(paste("latitude ",degree*N)),line=1.5,adj=0.5,padj=-1,cex = 1.2) 
  }
...
plot.lat.depth.GIF <- function(path.name,experiment.name,var,time.slice=9999.5,lon.slice=NULL,factor=1,name=NULL,
  ...
  int.matrix <- extr[lon.slice, , , time.level]
