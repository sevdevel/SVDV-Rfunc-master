...
plot.lat.depth <- function(path.name,experiment.name,var,time.slice=9999.5,lon.slice=NULL,factor=1,name=NULL,
  ...
  image2D(x = new.lat, y = new.depth*1e-3, z = new.int.matrix*factor, NAcol = "black", #log = "c",
...
plot.lon.lat <- function(path.name,experiment.name,var,time.slice=9999.5,depth.slice=NULL,factor=1,name=NULL,
  ...
  if (is.null(depth.slice)){
    ...
    #lat.width <- extract.lat.width(lat)
    #lon.width <- extract.lon.width(lon)
    ...
    for (j in 1:length(lon)){
      for (k in 1:length(lat)){
        ...
        #if (j == 1){lon.edge.left <- -180} else {lon.edge.left <- (-180 + sum(lon.width[1:(j-1)]))}
        #if (k == 1){lat.edge.left <- -90}  else {lat.edge.left <- (-90  + sum(lat.width[1:(k-1)]))}
        
        #lon.edge.right <- (-180 + sum(lon.width[1:j]))
        #lat.edge.right <- ( -90 + sum(lat.width[1:k]))
        
        #A.cell <- (convert.latlon.to.meters(lon1=lon.edge.left,lat1=lat.edge.left,lon2=lon.edge.left,lat2=lat.edge.right)*
        #            convert.latlon.to.meters(lon1=lon.edge.left,lat1=lat.edge.left,lon2=lon.edge.right,lat2=lat.edge.left)) # surface in m2
        
        #int.matrix[j,k] <- int.matrix[j,k]*1030*A # concentration in mol kg-1, 1030 kg m-3, surface in m2
